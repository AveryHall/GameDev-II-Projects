...
public class TargetCamera : MonoBehaviour {
	...
	void Start() {
		...
		Cursor.visible = false; //was Screen.showCursor
	...
	void Update() {
        ...
        //    go.GetComponent<GUIText>().text = "" + Time.time;
        //}

        if (editMode) {
            GameObject go = GameObject.Find("EditModeText");
		...
		if (Input.GetMouseButtonDown (0) || Input.GetMouseButtonDown(1)) {
			sh = new Shot();
			// Grab the posiStion and rotation of fpCamera
			...
			if (editMode) {
				if (Input.GetMouseButtonDown (0)) {
					...
					// RigSht button replaces the current shot
        ...
        //Press I to switch between editor mode and non-editor mode in game
    ...
    //    {
    //        if(playerRatings[i] < passingAccuracy)
    //        {
    //            return false;
    //       }
    //    }
