...
public class Deck : MonoBehaviour {
    ...
    public void ReadDeck(string deckXMLText) {
        ...
        // Grab an PT_XMLHashList of all of the <card>s in the XML file
        PT_XMLHashList xCardDefs = xmlr.xml["xml"][0]["card"];
        for (int i = 0; i < xCardDefs.Count; i++)
        {
            // For each of the <card>s create a new CardDefintion
    ...
    // Get the proper CardDefintion based on Rank (1 to 14 is Ace to King)
    ...
    private void AddDecorators(Card card) {
        ...
        foreach(Decorator deco in decorators) {
            ...
            // Set the localPosition  based on the location from DecjXML
    ...
    private void AddPips(Card card) {
        ...
        foreach(Decorator pip in card.def.pips) {
            // ...Instatiate a Sprite GameObject
            _tGO = Instantiate(prefabSprite) as GameObject;
    ...
    static public void Shuffle(ref List<Card> oCards) {             //a
        ...
        int ndx; // This will hold the new shuffle order
        tCards = new List<Card>(); // Initiaize the temporary List
